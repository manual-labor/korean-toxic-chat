...
한국어 인터넷 채팅을 **정상(normal)**과 **악성(toxic)**으로*분류하는*이진 분류 프로젝트이다.
...
본 저장소는 연구의 구현 코드와 실행에 필요한 파일을 제공한다. 데이터 구축 과정, 알고리즘의 이론적 배경 및 상세 실험 분석은 별도 제출 보고서에 기술하였다.
...
* `normal`: 일반적인 대화
* `review_needed`, `immediate_ban`: 욕설, 비하, 혐오, 공격적 표현 및 괴롭힘이 포함된 대화
...
따라서 일반 문서 중심으로 사전학습된 언어모델을 한국어 실시간 채팅 환경에 적용할 경우, 과업 특화 어휘와 비라벨 채팅 데이터를 이용한 추가 사전학습이 효과적인 방법이 될 수 있음을 확인하였다.
